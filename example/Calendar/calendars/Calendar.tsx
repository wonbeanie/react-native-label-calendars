...
import { labelType, sizeTypeType, dataDateType, nowDateType, onLabelData, defaultOptionType } from '../Calendars.d';

export default function Calendar({sizeType, labels, option, onLabelData, ...props} : propsType){
    ...
    let dayData = option.weekLangFormmat.length > 7 ? option.weekLangFormmat.splice(0,7) : option.weekLangFormmat;
    ...
    const dateElement = ({date, otherDate} : dateType, num : number, lastWeek : boolean, lastDate : boolean) => {
        ...
        if(otherDate){
            return (
                <TouchableOpacity key={num} style={touchableOpacityStyle} onPress={(e)=>{onPressOtherDate(date)}}>
        ...
        if(nowFormmetDate === fommetDate){
            return (
                <TouchableOpacity key={num} style={touchableOpacityStyle} onPress={(e)=>{onPressDate(e ,initDate, fommetDate)}}>
